...
| M31 | [Time-travel & branch sessions](milestones/M31-time-travel-branch.md) | **Done.** List checkpoints; fork to new `thread_id` via `update_state`; CLI `--list-checkpoints` / `--fork-from`; REPL `/rewind`. |
| M32 | [Parallel tools & fan-out](milestones/M32-parallel-tools-fanout.md) | **Done.** `PolicyToolNode`: parallel by default; serial A/B + ask-batch serial; error isolation; `--serial-tools`. |
| M33 | [Structured outputs & forced tool choice](milestones/M33-structured-output-tool-choice.md) | **Done.** Sidecar `invoke_structured` + `tool_choice`; CLI `--structured-route` / `--force-tool`; main ReAct unchanged. |
| M34 | [Observability traces (LangSmith / OTel)](milestones/M34-observability-traces.md) | **Done.** LangSmith env opt-in + `enrich_run_config`; `--trace-local` JSONL; redact helper; topology unchanged. |
...
| M39 | [Tool observation budgets](milestones/M39-tool-observation-budget.md) | **Done.** Unified post-tool size budget (head+tail); optional summarize-when-huge; `PolicyToolNode` gate. |
